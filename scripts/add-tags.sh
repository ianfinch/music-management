...
# Update the mp3 tags
docker run --rm -v"$dst":/home/appuser/audiofile guzo/audio-tools mid3v2 --delete-all audiofile
docker run --rm -v"$dst":/home/appuser/audiofile guzo/audio-tools mid3v2 --TALB "$album" audiofile \
...
if [[ "$albumArtist" == "Various Artists" ]] ; then
    ...
    docker run --rm -v"$dst":/home/appuser/audiofile guzo/audio-tools mid3v2 --TCMP 1 audiofile
fi

# For multi-CD sets, store CD number
if [[ "$cdNumber" != "" ]] ; then

    docker run --rm -v"$dst":/home/appuser/audiofile guzo/audio-tools mid3v2 --TPOS "$cdNumber" audiofile
fi
